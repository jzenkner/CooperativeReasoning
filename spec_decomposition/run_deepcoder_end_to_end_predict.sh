...
save_dir=../STG/stg_results/evaluation/deepcoder_${eval_run}_falsepositives_${compute_false_positives}/efficiency
...
# To use test data generated locally:
 base_data_dir=../STG/stg_data/deepcoder_data
...
if [[ "$rules" == "true" ]]; then
  experiment=NONE
  deepcoder_max_list_length=10
  num_examples=3
  num_test=25
  base_data_dir=./tiips_data/test_data_tfrecords/deepcoder
fi
...
if [[ "$nsa" == "true" ]]; then
  synthesizer_path_format=${base_model_dir}/joint_model/checkpoints/adr=0.1,ara=False,dr=0.1,e={experiment},ed=${embedding_dim},hd=${hidden_dim},l=0.0002,md=60,mpced=180,npb=32,s={seed},scnpr=0.0,ura=True/
fi
