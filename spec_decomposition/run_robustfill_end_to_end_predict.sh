...
base_data_dir=../STG/stg_data/robustfill_data
...
save_dir=./tiips_results/evaluation/robustfill_${eval_run}_falsepositives_${compute_false_positives}/efficiency
...
if [[ "$nsa" == "true" ]]; then
  synthesizer_path_format=${base_model_dir}/joint_model/checkpoints/adr=0.1,ara=False,dr=0.1,e={experiment},ed=${embedding_dim},hd=${hidden_dim},l=0.0002,md=200,mpced=800,npb=32,s={seed},scnpr=0.0,ura=True/
fi
...
for prediction_type in tiips; do
  ...
  echo "${synthesizer_max_distance}, ${synthesizer_max_program_cross_embed_distance}"
