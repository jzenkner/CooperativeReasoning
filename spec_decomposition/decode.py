...
def beam_search(inputs,
  ...
  def beam_search_loop_body_fn(state):
    ...
    if greedy:
      ...
      probs = jax.nn.softmax(flat_log_probs, axis=-1)
      
      # One RNG key per batch element
      rngs = jax.random.split(rng, batch_size)
      
      # Sample beams_to_keep indices for each batch
      topk_indices = jax.vmap(
          lambda r, p: jax.random.choice(
              r, flat_log_probs.shape[1], p=p, shape=(beams_to_keep,), replace=False
          )
      )(rngs, probs)
      
      # Gather the corresponding log probs
      topk_log_probs = jnp.take_along_axis(
          flat_log_probs, topk_indices, axis=1
      )
