  ...
  def step_fn(step):
    ...
    for name in factors:
      if name == 'constant':
        ...
        ret *= jnp.minimum(1.0, step / (warmup_steps * 16//FLAGS.per_device_batch_size))
      elif name == 'rsqrt_decay':
        ret /= jnp.sqrt(jnp.maximum(1.0, step - (warmup_steps * 16//FLAGS.per_device_batch_size)))
      elif name == 'rsqrt_normalized_decay':
        ret *= jnp.sqrt(warmup_steps)
        ret /= jnp.sqrt(jnp.maximum(step, (warmup_steps * 16//FLAGS.per_device_batch_size)))
      elif name == 'decay_every':
        ret *= (decay_factor**(step // (steps_per_decay * 16//FLAGS.per_device_batch_size)))
      elif name == 'cosine_decay':
        progress = jnp.maximum(0.0,
                               (step - (warmup_steps * 16//FLAGS.per_device_batch_size)) / float((steps_per_decay * 16//FLAGS.per_device_batch_size)))
        ret *= jnp.maximum(0.0,
...
def discard_last_subgoal(targets, sep_token=3, eos_token=2, pad_token=0):
    ...
    if targets.ndim == 3:
        # flatten leading dimension if needed
        targets = targets[0]
    
    seq_len = targets.shape[1] if targets.ndim == 2 else targets.shape[-1]

    def remove_last(row):
        # Find last EOS and last SEP
        arange = jnp.arange(row.shape[0])
        last_eos = jnp.max(jnp.where(row == eos_token, arange, -1))
        last_sep = jnp.max(jnp.where(row == sep_token, arange, -1))

        # set last SEP to EOS
        row = row.at[last_sep].set(eos_token)

        # create mask for positions to pad
        pad_mask = (arange > last_sep) & (arange <= last_eos)
        row = jnp.where(pad_mask, pad_token, row)
        return row

    # vectorize over batch
    out = jax.vmap(remove_last)(targets)
...
"""def discard_last_subgoal(targets, sep_token=3, eos_token=2, pad_token=0):
  ...
  def remove_last(row):
      # find all EOS tokens
      eos_index = jnp.where(row == eos_token)[0][-1]
      # find last start token before last EOS
      last_start = jnp.where(row == sep_token)[0][-1]
      row = row.at[last_start].set(eos_token)
      row = row.at[last_start+1:eos_index+1].set(pad_token)
      return row

  # flatten batch dims
  out = jnp.expand_dims(jnp.stack([remove_last(row) for row in targets[0]]), axis=0)
  # pad back to same rank if needed (optional)
  return out"""

def main(_):
  """import os

  data_dir = os.path.join(FLAGS.dataset_dir, f'{FLAGS.experiment}_data')
        ...
        if FLAGS.model_type == 'baseline_model':
          ...
          if FLAGS.dataset_type == 'lambdabeam':
            ...
            return deepcoder_dsl.Statement.from_str(statement_str,
                                                     check_variable_name=False)
      except (operation_base.ParseError, deepcoder_dsl.RunError):
        return None  # Program does not compile.
  ...
  for step in range(start_step, FLAGS.num_train_steps):
    ...
    if not FLAGS.predict_only:
      ...
      if FLAGS.compute_false_positives:
        ...
        inputs, outputs = inputs[:, :, :FLAGS.num_examples-1, :], outputs[:, :, :FLAGS.num_examples-1, :]
        if FLAGS.model_type == 'spec_decomposer_model':
          targets = discard_last_subgoal(targets)
    ...
    if (step and step % FLAGS.predict_freq == 0) or is_last_step:
      ...
      for dataset, predict_or_test in [(predict_ds, 'predict')]:#  (test_ds, 'test')] todo uncomment when test data is available
        ...
        for beam_size in [1, 10]:
          ...
          for batches in dataset.as_numpy_iterator():
            ...
            if FLAGS.compute_false_positives:
              # FIXME: this and all the above copies are not very efficient because we only need this for the beam metrics of the synthesis models
              # Extract held-out test sample for evaluation of program correctness
              # Finally, we're using the held-out test samples to determine success
              inputs, outputs = inputs[:, :, :FLAGS.num_examples-1, :], outputs[:, :, :FLAGS.num_examples-1, :]
              if FLAGS.model_type == 'spec_decomposer_model':
                targets = discard_last_subgoal(targets)
