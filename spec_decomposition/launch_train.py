...
MAX_LIST_LENGTH = flags.DEFINE_integer(
    'max_list_length', 5,
    'The maximum length of a LambdaBeam list input.')
MAX_INT = flags.DEFINE_integer(
    'max_int', 50,
    'The maximum value of a LambdaBeam int.')
MAX_CONST = flags.DEFINE_integer(
    'max_const', 5,
    'The maximum value of a LambdaBeam int constant.')
...
def compute_lengths():
  ...
  if _DATASET_TYPE.value == 'lambdabeam' or _DATASET_TYPE.value == 'deepcoder':
    ...
    # list elements.
    object_token_length = MAX_LIST_LENGTH.value + 5
  ...
  logging.info(f'For dataset {_DATASET_TYPE.value} and model {_MODEL_TYPE.value}:')
  logging.info(f'  max_input_length = {max_input_length}')
  logging.info(f'  predict_max_input_length = {predict_max_input_length}')
  logging.info(f'  max_target_length = {max_target_length}')
  logging.info(f'  max_distance_options = {max_distance_options}')
  logging.info(f'  max_program_cross_embed_distance_options = '
...
def main(_):
  ...
  static_args = collections.OrderedDict([
      ...
      ('num_heads', 4),
      ('num_layers', 3),
      # LambdaBeam-specific settings.
      ('max_list_length', MAX_LIST_LENGTH.value),
      ('max_int', MAX_INT.value),
      ('max_const', MAX_CONST.value),
      # Training settings
      ('compute_false_positives', _COMPUTE_FALSE_POSITIVES.value)
  ])
