...
class DatasetTest(parameterized.TestCase):
  ...
  def test_random_inputs(self, max_length, max_int, num_inputs, num_examples):
    with flagsaver.flagsaver(max_list_length=max_length,
                             max_int=max_int):
      ...
      for inputs in example_inputs:
        ...
        self.assertTrue(all(dsl.validate_result(x) for x in inputs))
  ...
  def test_random_new_variable(self):
    x4 = dsl.variable_token(4)
    existing_variables = list(dsl.ALL_VARIABLES)
  ...
  def test_random_new_variable_raises(self):
    for ordered in [True, False]:
      with self.assertRaises(ValueError):
        old_sample_random.random_new_variable(dsl.ALL_VARIABLES,
                                              ordered=ordered)
  ...
  def test_is_valid_operation(self, operation, var_dict, expected):
    op = dsl.TOKEN_TO_OPERATION[operation]
  ...
  def test_is_redundant(self, states, expected):
    program_states = [dsl.ProgramState.from_str(state)
                      for state in states]
  ...
  def test_has_dead_code(self, program_str, expected):
    program = dsl.Program.from_str(program_str)
  ...
  @parameterized.named_parameters(
      ...
      ('always_all_zeros',
       ('x3 = INPUT | x8 = ZipWith Subtract x3 x3 | x5 = Minimum x3 '
        '| x4 = Take x5 x8'),
       True),
  )
