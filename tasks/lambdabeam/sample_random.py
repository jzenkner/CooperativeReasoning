...
from tasks.lambdabeam.lambdabeam_dsl import validate_result
from tasks.lambdabeam import lambdabeam_dsl as dsl
from tasks.lambdabeam import old_sample_random
from tasks.operation_base import OperationBase, LambdaBeamError
from tasks import value
...
def has_constant_output(states):
  ...
  if len(outputs) != len(states):
    raise LambdaBeamError('Random program should execute on all inputs.')
...
def _get_next_statements(program_state,
                         ...
                         lambdas=dsl.TOKEN_TO_LAMBDA.values(),
  ...
  valid_operations = [op for op in operations if op.output_type in (int, list) and
  ...
  for op in valid_operations:
    ...
    for t in op.inputs_type:
      if isinstance(t, tuple):  # Argument is a lambda.
        ...
        lambda_calls = []
        for random_lambda in valid_lambdas:
          lambda_args, free_vars = [], []

          if isinstance(op, dsl.HigherOrderLambdaBeamOperation):
            if op.token in {'ZipWith', 'Scanl1'}:
              free_vars = [dsl.FreeVariable(f"v{j + 1}") for j in range(2)]
              lambda_args += free_vars
            else:
              ...
              for inp_index, inp_type in enumerate(random_lambda.inputs_type):
                if inp_type == int:
                  if inp_index == 0 and random_lambda.token not in ['IsOdd', 'IsEven', 'Equal']:
                    ...
                    # TODO: also use the integer intput variable as an argument to lambdas
                    int_input_var = None
                    for idx, ele in enumerate(program_state.state):
                      if type(ele) == int:
                        int_input_var = program_state.variables[idx]

                    # TODO: do this only of 50% of all cases
                    if int_input_var and inp_index == 1 and not isinstance(lambda_args[0], value.ConstantValue):
                      fv = value.ConstantValue(int_input_var)
                      lambda_args.append(fv)
                    else:
                      lambda_args.append(dsl.ConstantValue(
                        random.randint(dsl.lambdabeam_min_const(),
                                       dsl.lambdabeam_max_const())
                      ))

                elif inp_type == list:
                  var_name = random.choice(var_dict[list])
                  fv = dsl.FreeVariable(var_name)
                  lambda_args.append(fv)
                  free_vars.append(fv)
                else:
                  raise NotImplementedError(f"Lambda arg type {inp_type}")
          # Build the lambda call expression.
          lb_call = random_lambda.apply(lambda_args, free_variables=free_vars)
          lambda_calls.append(lb_call)

        # Add list of all possible lambda call expressions for this argument slot.
        product_args.append(lambda_calls)
...
def _sample_program_helper(
  ...
  for statement in _get_next_statements(
    ...
    if not all(validate_result(ns.get_output()) for ns in next_states):
      continue
    ...
    if experiment == exp_module.Experiment.COMPOSE_DIFFERENT_CONCEPTS:
      ...
      keep_fn = lambda program: (  # pylint: disable=g-long-lambda
          any(f'Scanl1 {lambda_token}' in str(program)
              for lambda_token in ['Max', 'Add', 'Multiply']))
    else:
      operations = [dsl.get_operations()]
      lambdas = dsl.TOKEN_TO_LAMBDA.values()
      keep_fn = None
