...
from tasks.operation_base import RunError, ParseError, LambdaBeamError
from tasks.lambdabeam import lambdabeam_dsl as dsl
from tasks.value import FreeVariable, ConstantValue, InputVariable

FLAGS = flags.FLAGS


class LambdaBeamDslTest(parameterized.TestCase):

  def setUp(self):
    super(LambdaBeamDslTest, self).setUp()
    self.head_op = dsl.TOKEN_TO_OPERATION['Head']
    self.map_op = dsl.TOKEN_TO_OPERATION['Map']
    self.plus_one_lambda = dsl.TOKEN_TO_LAMBDA['Add'].apply([FreeVariable('v1'), ConstantValue(1)], free_variables=[FreeVariable('v1')])
    self.square_lambda = dsl.TOKEN_TO_LAMBDA['Square'].apply([FreeVariable('v1')], free_variables=[FreeVariable('v1')])
    self.times_3_lambda = dsl.TOKEN_TO_LAMBDA['Multiply'].apply([FreeVariable('v1'), ConstantValue(3)], free_variables=[FreeVariable('v1')])
    self._saved_flags = flagsaver.save_flag_values()

  def tearDown(self):
    flagsaver.restore_flag_values(self._saved_flags)
    super(LambdaBeamDslTest, self).tearDown()
  ...
  def test_tokenize_result_raises(self, result):
    with self.assertRaises(LambdaBeamError):
  ...
  def test_operation_run(self):
    ...
    self.assertEqual(self.map_op.run(
        [self.plus_one_lambda, InputVariable([[7, 2, 3]], 'x0')]), [8, 3, 4])
    # Result is too big.
    self.assertIsNone(self.map_op.run([self.square_lambda, InputVariable([[20]], 'x0')]))
    # self.assertIsNone(self.square_lambda.run([[20]]))

  def test_operation_run_raises(self):
    with self.assertRaises(RunError):
      self.head_op.run([[7, 2, 3], 0])  # Wrong arity.
    with self.assertRaises(RunError):
      self.map_op.run([0, InputVariable([[7, 2, 3]], 'x0')])  # First arg should be lambda.
  ...
  def test_statement(self):
    ...
    statement_2 = dsl.Statement.from_str('x0 = Map Add 1 x1')
    self.assertEqual(statement_2.tokenize(), ['x0', '=', 'Map', 'Add', '1', 'x1'] )
  ...
  @parameterized.named_parameters(
      ...
      ('bad_lhs_variable', 'y1 = Map Add 1 x0'),
      ('bad_equals', 'x1 == Head x0'),
      ('unknown_operation', 'x1 = NotAnOp x0'),
      ('unexpected_lambda_head', 'x1 = Head Add 1 x0'),
      ('unexpected_lambda_map', 'x1 = Map Add 1 Add 1'),
      ('needs_lambda_got_variable', 'x2 = Map x1 x0'),
      ('needs_lambda_got_operation', 'x1 = Map Map x0'),
      ('lhs_as_arg', 'x1 = Map Add 1 x1'),
      ('bad_arg_variable', 'x1 = Map Add 1 y0'),
  ...
  def test_statement_run(self):
    ...
    self.assertIsNone(statement.run(bad_initial_state).get_output())
  ...
  def test_program(self):
    ...
    program_2 = dsl.Program.from_str(
        'x0 = INPUT | x1 = INPUT | x2 = Reverse x1 | x3 = ZipWith Add x0 x2')
    ...
    self.assertIsNone(program_3.run([[]]).get_output())
  ...
  def test_first_order_operations(self, token, inputs, expected):
    ...
    assert token not in dsl.TOKEN_TO_LAMBDA.keys()
  ...
  @parameterized.parameters(
      ('Map', 'Add', 1, [InputVariable([[5, 6, 7]], 'x0')], [6, 7, 8]),
      ('Map', 'Add', 1, [InputVariable([[-4]], 'x0')], [-3]),
      ('Map', 'Add', 1, [InputVariable([[]], 'x0')], []),
      ('Map', 'Add', None, [InputVariable([[5, 6, 7]], 'x0'), InputVariable([1], 'x1')], [6, 7, 8]),
      ('Map', 'Subtract', 1, [InputVariable([[5, 2, 7]], 'x0')], [4, 1, 6]),
      ('Map', 'Multiply', 2, [InputVariable([[2, 0, 3, 1]], 'x0')], [4, 0, 6, 2]),
      ('Map', 'IntDivide', 2, [InputVariable([[4, 3, 0, 7, 6, -3]], 'x0')], [2, 1, 0, 3, 3, -2]),
      ('Map', 'Multiply', -1, [InputVariable([[4, -6, 0]], 'x0')], [-4, 6, 0]),
      ('Map', 'Multiply', 3, [InputVariable([[1, 3, 0]], 'x0')], [3, 9, 0]),
      ('Map', 'IntDivide', 3, [InputVariable([[-6, -5, 0, 3, 4, 7]], 'x0')], [-2, -2, 0, 1, 1, 2]),
      ('Map', 'Multiply', 4, [InputVariable([[2]], 'x0')], [8]),
      ('Map', 'IntDivide', 4, [InputVariable([[8, 1, 0, -1]], 'x0')], [2, 0, 0, -1]),
      ('Filter', 'Greater', 0, [InputVariable([[4, -1, 0, 2, -4]], 'x0')], [4, 2]),
      ('Filter', 'IsEven', None, [InputVariable([[4, -1, 0, 2, -4]], 'x0')], [4, 0, 2, -4]),
      ('Count', 'Greater', 0, [InputVariable([[4, -1, 0, 2, -4]], 'x0')], 2),
      ('Count', 'IsOdd', None, [InputVariable([[4, -1, 0, 2, -4]], 'x0')], 1),
      ('ZipWith', 'Subtract', None, [InputVariable([[3, 2, 5]], 'x0'), InputVariable([[-2, 4, 1]], 'x1')], [5, -2, 4]),
      ('ZipWith', 'Multiply', None, [InputVariable([[3, 2, 5]], 'x0'), InputVariable([[-2, 4, 1, 3]], 'x1')], [-6, 8, 5]),
      ('ZipWith', 'Min', None, [InputVariable([[3, 2, 5, 0]], 'x0'), InputVariable([[-2, 4, 1]], 'x1')], [-2, 2, 1]),
      ('ZipWith', 'Add', None, [InputVariable([[]], 'x0'), InputVariable([[1]], 'x1')], []),
      ('Scanl1', 'Add', None, [InputVariable([[]], 'x0')], []),
      ('Scanl1', 'Add', None, [InputVariable([[6]], 'x0')], [6]),
      ('Scanl1', 'Add', None, [InputVariable([[6, -2, -5, 3]], 'x0')], [6, 4, -1, 2]),
      ('Scanl1', 'Max', None, [InputVariable([[-3, 2, -1, 3, 2, 5]], 'x0')], [-3, 2, 2, 3, 3, 5]),
      ('If', 'Greater', 0, [InputVariable([[-1, 2, -3]], 'x0'), InputVariable([[5, 6, 7]], 'x1'), InputVariable([[10, 20, 30]], 'x2')], [10, 2, 30])
  )
  def test_higher_order_operations(self, op_token, lambda_token, constant, inputs,
                                   expected):
    op = dsl.TOKEN_TO_OPERATION[op_token]
    assert (op_token not in dsl.TOKEN_TO_LAMBDA.keys()) and any(op.num_bound_variables)
    v1 = FreeVariable('v1')
    lambda_op = dsl.TOKEN_TO_LAMBDA[lambda_token]
    self.assertEqual((lambda_op.inputs_type, lambda_op.output_type),
                     op.inputs_type[0])
    self.assertLen(inputs, op.arity - 1)
    self.assertLen(op.inputs_type, op.arity)
    for inp, inp_type in zip(inputs, op.inputs_type[1:]):
      self.assertEqual(inp.type, inp_type)

    num_free_vars = len(inputs)
    if op_token == 'Scanl1':
      num_free_vars += 1
    ipts = [v1, ConstantValue(constant)] if constant is not None else [FreeVariable(f"v{j+1}") for j in range(num_free_vars)]

    lambda_object = lambda_op.apply(ipts, free_variables=[FreeVariable(f"v{j+1}") for j in range(num_free_vars)])
    result = op.run([lambda_object] + inputs)
    self.assertEqual(result, expected)
    if result is not None:
      self.assertEqual(type(result), op.output_type)
  ...
  def test_vocab_tables(self):
    ...
    program = dsl.Program.from_str(
        'x0 = INPUT | x9 = INPUT | x2 = Reverse x9 | x5 = ZipWith Add x0 x2')
