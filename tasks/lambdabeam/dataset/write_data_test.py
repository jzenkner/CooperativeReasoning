...
from tasks.lambdabeam.dataset import write_data
...
class WriteDataTest(parameterized.TestCase):
  ...
  def test_serialize_decomposition_examples(self):
    ...
    program = lambdabeam_dsl.Program.from_str(
        'x5 = INPUT | x1 = INPUT | x2 = Map Add 1 x5 | x7 = Take x1 x2')
    examples = [lambdabeam_dsl.Example([[4, 1, 3], 2], [5, 2]),
                lambdabeam_dsl.Example([[2, 5, 8, 7], 3], [3, 6, 9])]
    task = lambdabeam_dsl.ProgramTask(program, examples)
            ...
            feature={
                ...
                'corrupted_next_part':
                    ...
                    write_data._bytes_feature(
                        ...
                        .bytes_list.value),
                'program_part':
                    write_data._bytes_feature(['x2 = Map Add 1 x5']),
            }))
