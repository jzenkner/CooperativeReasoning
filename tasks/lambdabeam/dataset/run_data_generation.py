...
_MAX_LIST_LENGTH = flags.DEFINE_integer(
    'max_list_length', 5,
    'The maximum length of a list input.')
_MAX_INT = flags.DEFINE_integer(
    'max_int', 50,
    'The maximum value of a int.')
...
def run_job(job_args):
  subprocess.run(
      args=(['python', '-m', 'tasks.lambdabeam.dataset.write_data']
...
def main(_):
  ...
  static_args = collections.OrderedDict([
      ...
      ('max_list_length', _MAX_LIST_LENGTH.value),
      ('max_int', _MAX_INT.value),
      ('max_const',_MAX_CONST.value),
  ])
