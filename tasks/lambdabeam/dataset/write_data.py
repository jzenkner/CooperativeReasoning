...
from tasks.lambdabeam import sample_random
...
_SAVE_DIR = flags.DEFINE_string(
    'save_dir', '/tmp/decomposition/lambdabeam', 'Directory to save results to.')
    ...
    if technique == 'copy_output':
      ...
      if type(results[0]) == int:  # pylint: disable=unidiomatic-typecheck
        ...
        for result in changed:
          ...
          assert all(isinstance(x, int) for x in result), result

          result = typing.cast(list[int], result)  # Not an int or None.
          # Perturbed numbers should be chosen reasonably considering the
          # existing numbers.
          min_result = min(result) if result else dsl.lambdabeam_min_int()
          max_result = max(result) if result else dsl.lambdabeam_max_int()
          range_expansion = max(5, (max_result - min_result) // 2)
          min_perturb = max(dsl.lambdabeam_min_int(),
                            min_result - range_expansion)
          max_perturb = min(dsl.lambdabeam_max_int(),
...
def count_records(fname):
  try:
    ...
    return ds.reduce(tf.constant(0), lambda x, _: x + 1).numpy()
...
def main(_):
  ...
  # check if file exists and contains n valid data
  skip = False
  if (os.path.exists(entire_programs_fname) and
      os.path.exists(decomposition_data_fname)):

    ep_count = count_records(entire_programs_fname)
    ddf_count = count_records(decomposition_data_fname)

    # Skip only if entire_programs matches exactly
    if ep_count > 0 and ddf_count > 0:
      logging.info(f"[SKIP] Files already contain {ep_count} valid files → {entire_programs_fname}")
      skip = True
    else:
      logging.info(f"[REGENERATE] Files exist but are incomplete or corrupted. {ep_count}, {ddf_count}")
  else:
    logging.info("[GENERATE] Files do not exist; generating data.")

  if skip:
    return

  # Otherwise: generate new data
  with tf.io.TFRecordWriter(entire_programs_fname) as entire_writer, \
      tf.io.TFRecordWriter(decomposition_data_fname) as decomp_writer:
    for i in range(_NUM_SEARCHES.value):
      ...
      for task in tasks:
        entire_writer.write(serialize_entire_program_example(task))

        for example in serialize_decomposition_examples(task):
          decomp_writer.write(example)

  logging.info(f'Wrote entire programs data to {entire_programs_fname}')
  logging.info(f'Wrote decomposition data to {decomposition_data_fname}')
