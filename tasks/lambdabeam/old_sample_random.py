...
"""Generates random programs in Lambdabeam DSL."""
...
def random_inputs(num_examples: int, num_inputs: int,
  ...
  int_range_options = [
      ...
      if dsl.lambdabeam_min_int() < x and y < dsl.lambdabeam_max_int()
  ]
  int_range_options.append((dsl.lambdabeam_min_int(), dsl.lambdabeam_max_int()))
  list_int_ranges = random.choices(int_range_options, k=num_inputs)
  length_range_options = _get_length_range_options(
      dsl.lambdabeam_max_list_length())
  ...
  while len(all_examples) < num_examples:
    ...
    for i, type_, in enumerate(types):
      if type_ == int:
        ...
        max_int = dsl.lambdabeam_max_list_length()
...
def is_valid_operation(operation: operation_base.OperationBase,
  ...
  if isinstance(operation, dsl.FirstOrderLambdaBeamOperation):
    start_idx = 0
  elif isinstance(operation, dsl.HigherOrderLambdaBeamOperation):
    start_idx = 1
  else:
    raise operation_base.LambdaBeamError(f'Unhandled operation {operation} of type: {type(operation)}')
...
def random_statement(program_state: dsl.ProgramState,
                     ...
                     operations: List[operation_base.OperationBase],
                     lambdas: List[dsl.LambdaOperation],
  ...
  valid_operations = [op for op in operations if op.output_type in (int, list) and is_valid_operation(op, var_dict)]
  ...
  for t in random_op.inputs_type:
    if isinstance(t, tuple):  # Argument is a lambda.
      ...
      random_lambda = random.choice([
        l for l in valid_lambdas if (l.inputs_type, l.output_type) == t
      ])

      lambda_args = []
      free_vars = []

      if isinstance(random_op, dsl.HigherOrderLambdaBeamOperation):
        if random_op.token in {'ZipWith', 'Scanl1'}:
          free_vars = [FreeVariable(f"v{j+1}") for j in range(2)]
          lambda_args += free_vars
        else:
          # Map, Filter, Count, etc. -> can parameterize with constants or FreeVariables
          for inp_index, inp_type in enumerate(random_lambda.inputs_type):
            if inp_type == int:
              if inp_index == 0 and random_lambda.token not in ['IsOdd', 'IsEven', 'Equal']:
                # First argument must be a free variable
                fv = dsl.FreeVariable(f"v{inp_index+1}")
                lambda_args.append(fv)
                free_vars.append(fv)
              else:
                # Second argument is always a constant
                lambda_args.append(dsl.ConstantValue(
                  random.randint(dsl.lambdabeam_min_int(),
                                 dsl.lambdabeam_max_int())
                ))
            elif inp_type == list:
              var_name = random.choice(var_dict[list])
              lambda_args.append(dsl.FreeVariable(var_name))
              free_vars.append(dsl.FreeVariable(var_name))
            else:
              raise NotImplementedError(f"Lambda arg type {inp_type}")

      # Wrap in OperationValue for the statement
      lb_call = random_lambda.apply(lambda_args, free_variables=free_vars)
      args.append(lb_call)
...
def has_duplicate_output(program: dsl.Program,
  ...
  if len(outputs) != len(example_inputs):
    raise operation_base.LambdaBeamError('Random program should execute on all inputs.')
...
def _sample_program(
  ...
  states = [dsl.ProgramState(inputs, input_variables) for inputs in example_inputs]
  statements = []
  ...
  for i in range(num_statements):
    if experiment == exp_module.Experiment.SWITCH_CONCEPT_ORDER:
      ...
      if is_train:
        operations = (dsl.HIGHER_ORDER_OPERATIONS.values() if i < num_statements // 2
                      else dsl.FIRST_ORDER_OPERATIONS.values())
      else:
        operations = (dsl.FIRST_ORDER_OPERATIONS.values() if i < num_statements // 2
                      else dsl.HIGHER_ORDER_OPERATIONS.values())
...
def random_task(
    ...
    operations: Optional[List[operation_base.OperationBase]] = None,
    lambdas: Optional[List[dsl.LambdaOperation]] = None,
  ...
  if operations is None:
    operations = list(dsl.TOKEN_TO_OPERATION.values()).copy()
  if lambdas is None:
    lambdas = list(dsl.TOKEN_TO_LAMBDA.values()).copy()
