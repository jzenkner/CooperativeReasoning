...
from spec_decomposition import variables as variables_module
from tasks import value as value_module
from tasks.value import ConstantValue, Value
...
class LambdaBeamError(Exception):
...
class OperationBase(abc.ABC):
  ...
  def set_bound_vars(self, num_bound_variables):
    self.num_bound_variables = num_bound_variables
    self.bound_variables = [[value_module.get_bound_variable(i)
                             for i in range(num_bound)]
                            for num_bound in num_bound_variables]

    assert len(self.bound_variables) == self.arity
    self.has_any_bound_variables = bool(sum(self.bound_variables, []))
  ...
  def apply(self, arg_values, arg_variables=None, free_variables=None):
    ...
    if not all(isinstance(v, Value) for v in arg_values):
      raise RunError(
        f'Encountered argument that is not a value: {arg_values}')
