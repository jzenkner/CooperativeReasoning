...
RESULT_DIR = os.path.expanduser('./tiips_data/false_positivestrue')
...
def main(argv: Sequence[str]) -> None:
  ...
  for dataset in sorted(os.listdir(TEST_DATA_DIR)):
    if 'lambdabeam' not in dataset:
      continue
    print(f'Processing {dataset=}')

    for filename in sorted(os.listdir(os.path.join(TEST_DATA_DIR, dataset))):
      print(f'  Processing {filename=}')
      if filename != 'LAMBDABEAM_HANDWRITTEN.jsonl':
        continue

      print(os.path.join(TEST_DATA_DIR, dataset, filename))
      ...
      this_result_dir = os.path.join(
          RESULT_DIR, dataset + '_data', generalization_task + '_data')
  ...
  for example in tf.data.TFRecordDataset(out_filename):
      ex = tf.train.Example()
      ex.ParseFromString(example.numpy())
      print(ex)
      break
