...
from models import relative_attention
...
class TransformerDecoder(nn.Module):
  ...
  def __call__(self,
    ...
    if cfg.shift:
      ...
      #jax.debug.print("y after shift min/max/NaN: [{}, {}], {}", jnp.nanmin(y), jnp.nanmax(y), jnp.isnan(y).any())

    y = output_embed(y)
    #jax.debug.print("y embed min/max/NaN: [{}, {}], {}", jnp.nanmin(y), jnp.nanmax(y), jnp.isnan(y).any())
    ...
    #jax.debug.print("decoder input before block min/max/NaN: [{}, {}], {}", jnp.nanmin(y), jnp.nanmax(y),
    #                jnp.isnan(y).any())
    ...
    #jax.debug.print("decoder output after block {} min/max/NaN: [{}, {}], {}", lyr, jnp.nanmin(y), jnp.nanmax(y),
    #                jnp.isnan(y).any())
...
class TransformerIOEncoder(nn.Module):
  ...
  def __call__(self,
    ...
    #jax.debug.print("decoder input min/max/NaN: [{}, {}], {}", jnp.min(inputs), jnp.max(inputs),
    #                jnp.isnan(inputs).any())
    ...
    # Embed inputs.
    x = embed(x)
    #jax.debug.print("encoder embed min/max/NaN: [{}, {}], {}", jnp.nanmin(x), jnp.nanmax(x), jnp.isnan(x).any())
    if not cfg.use_relative_attention:
